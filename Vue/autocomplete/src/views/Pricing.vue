    ...
    <div class="w-1/2 bg-blue-900 rounded-lg shadow px-6 py-12 flex flex-col items-center">
      <div class="bg-gray-400 rounded-full flex justify-around p-1">
        <button @click="currentFrequency = frequency"
                v-for="(price, frequency) in plans[0].pricing" v-bind:key="(price, frequency)"
                class="rounded-full text-xs font-bold px-6 py-1 ml-1 uppercase focus:outline-none"
                :class="currentFrequency == frequency ? 'bg-blue-600 text-gray-200' : '' ">
                {{frequency}}</button>
      </div>

      <!-- List informations about packs -->
      <div class="flex w-full pt-8"> <!-- "pt" increases the space between the upper div and this one -->
...
export default {
  ...
  methods: {
    getPrice (price) {
      // Calls the methods currencyUSD or currencyEUR
